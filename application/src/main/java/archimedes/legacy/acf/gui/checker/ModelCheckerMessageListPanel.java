...
public class ModelCheckerMessageListPanel extends JPanel implements ActionListener, MouseListener {

	private static final String RES_MODEL_CHECKER_BUTTON_CLOSE_TITLE = "ModelChecker.button.close.title";
	private static final String RES_MODEL_CHECKER_BUTTON_GENERATE_TITLE = "ModelChecker.button.generate.title";
	 ...
	 * @changed OLI 11.07.2016 - Added.
	 */
	public ModelCheckerMessageListPanel(GUIBundle guiBundle, ModelCheckerMessage[] mcms, boolean checkBeforeGenerate,
			ModelCheckerMessageListEventFirer eventFirer, ModelCheckerMessageListFrameListener... listeners) {
	...
	private JPanel createMainPanel() {
		...
		this.tableWarningsAndErrors = new JTableWithInifile(
				new ModelCheckerMessageTableModel(this.mcms, this.guiBundle), this.guiBundle.getInifile(),
				"ModelChecker.MessageTable");
		this.tableWarningsAndErrors.addMouseListener(this);
		this.tableWarningsAndErrors.restoreFromIni();
		this.tableWarningsAndErrors.setDefaultRenderer(Object.class, new ModelCheckerMessageTableCellRenderer());
	...
	private JPanel createButtonPanel() {
		...
		if (this.checkBeforeGenerate) {
			this.buttonGenerate = this.guiBundle.createButton(RES_MODEL_CHECKER_BUTTON_GENERATE_TITLE, "generate", this,
					p);
		...
		this.buttonClose = this.guiBundle.createButton(RES_MODEL_CHECKER_BUTTON_CLOSE_TITLE, "close", this, p);
		return p;
	}
	...
	public void updateMessages(ModelCheckerMessage[] mcms) {
		this.tableWarningsAndErrors.setModel(new ModelCheckerMessageTableModel(mcms, this.guiBundle));
		this.tableWarningsAndErrors.restoreFromIni();
		this.mcms = mcms;
	}
