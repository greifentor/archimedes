...
abstract public class AbstractResourceUpdater implements ResourceUpdater {
     ...
     */
    @Override public void updateResources(ResourceData[] rids, IndividualPreferences ip,
            DataModel dm) {
        if (rids.length == 0) {
            return;
        }
        ...
        try {
            ...
            for (ResourceData rid : rids) {
                if (p.get(rid.getResourceId()) == null) {
                    ...
                    additions += "\n" + rid.getResourceId() + "=" + this.escapeUnicode(
                            rid.getDefaultValue());
                }
            ...
            LOG.error("error while reading resource file: "
                    + new File(fileName).getAbsolutePath() + ", error: " + e.getMessage());
        }
    }
