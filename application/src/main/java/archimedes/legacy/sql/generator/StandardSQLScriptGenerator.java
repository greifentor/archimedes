...
/**
 * A SQL script generator for "Standard" SQL. It can be used as base for other SQL script generator implementations.
 *
 * @author O.Lieshoff
 *
...
public class StandardSQLScriptGenerator implements SQLScriptGenerator {
	...
	protected String transferCreateTableToSQLStatement(CreateTable change) {
		...
		for (MetaDataColumn c : columns) {
			...
			/*
			 * if (this.notNullConstraints && (c.isNotNull() || c.isPrimaryKey())) { s += " NOT NULL"; }
			 */
	...
	protected String transferDropPrimaryKeyConstraintToSQLStatement(DropPrimaryKeyConstraint change) {
		...
		if (t.getPrimaryKeyMembers().length == 0) {
			System.out.println(
					"WARNING: PRIMARY KEY REQUIRED ALTHOUGH NO PRIMARY KEY IS " + "DEFINED FOR TABLE: " + t.getName());
			return "";
		}
