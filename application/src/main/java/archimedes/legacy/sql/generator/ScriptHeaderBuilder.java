...
public class ScriptHeaderBuilder {
	 ...
	 * @param dbName     The name of the database or <CODE>null</CODE> if there is no name for the database.
	 ...
	 * @changed OLI 16.12.2015 - Added.
	 */
	public ScriptHeaderBuilder(DBExecMode dbMode, String dbName, String schemeName, PTimestamp timestamp,
			String version, UserInformation userInfos) {
		super();
	...
	protected String build() {
		...
		sql.append(" * Created by:  " + this.userInfos.getUserName() + " (" + this.userInfos.getUserToken() + ")\n");
		...
		if ((this.dbMode == DBExecMode.POSTGRESQL) && (this.schemeName != null)) {
			...
			sql.append("\n");
			sql.append("COMMENT ON SCHEMA \"" + this.schemeName + "\" IS 'Version " + this.version + "';\n");
		}
		return sql.toString();
	}
