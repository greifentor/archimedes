...
public class ViewXMLBuilder extends AbstractXMLBuilder {
	...
	public XMLNode buildNodes(ViewModel[] views, XMLNodeFactory xmlNodeFactory) {
		...
		for (ViewModel vm : views) {
			...
			this.addAttribute(viewNode, "hideTechnicalFields", String.valueOf(vm.isHideTechnicalFields()));
			this.addAttribute(viewNode, "hideTransientFields", String.valueOf(vm.isHideTransientFields()));
			this.addAttribute(viewNode, "mainView", String.valueOf(vm.isMainView()));
			this.addAttribute(viewNode, "showReferencedColumns", String.valueOf(vm.isShowReferencedColumns()));
