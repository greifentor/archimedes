	...
	public XMLNode buildNodes(TableModel[] tables, XMLNodeFactory xmlNodeFactory) {
		...
		for (TableModel t : tables) {
			...
			for (ColumnModel c : t.getColumns()) {
				...
				this.addAttribute(node, "panelNumber", String.valueOf(t.getPanelPosition(c.getPanel())));
				...
				if (c.getRelation() != null) {
					...
					for (GUIViewModel gv : ((GUIRelationModel) r).getViews()) {
						...
						this.addAttribute(view, "direction0",
								String.valueOf(((GUIRelationModel) r).getDirection((GUIViewModel) v, 0).toString()));
						this.addAttribute(view, "direction1",
								String.valueOf(((GUIRelationModel) r).getDirection((GUIViewModel) v, 1).toString()));
						this.addAttribute(view, "name", v.getName());
						this.addAttribute(view, "offset0",
								String.valueOf(((GUIRelationModel) r).getOffset((GUIViewModel) v, 0)));
						this.addAttribute(view, "offset1",
				...
				this.addAttribute(node, "sequenceForKeyGeneration",
						(c.getSequenceForKeyGeneration() != null ? c.getSequenceForKeyGeneration().getName() : ""));
				this.addAttribute(node, "suppressForeignKeyConstraints",
			...
			for (NReferenceModel nr : t.getNReferences()) {
				...
				this.addAttribute(compareToNode, "nReferencePanelType",
						String.valueOf(nr.getNReferencePanelType().name()));
				this.addAttribute(compareToNode, "panelNumber", String.valueOf(nr.getPanel().getPanelNumber()));
