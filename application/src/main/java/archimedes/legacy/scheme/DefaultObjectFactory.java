...
public class DefaultObjectFactory implements ObjectFactory {
	...
	private void loadTable(Properties p, TabellenModel tm, DiagrammModel d, ViewModel view) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (name.length() > 0) {
				...
				try {
					...
					stm
							.setDoNotPrint(Boolean.parseBoolean(p.getProperty("template.stereotype." + i
									+ ".isdonotprint")));
		...
		/*
		 * len = this.getInteger(p, "template.stereotype.count", 0); for (int i
		 * = 0; i < len; i++) { String name =
		 * p.getProperty("template.stereotype." + i + ".name", ""); if
		 * (name.length() > 0) { StereotypeModel stm = null; try { stm =
		 * d.getStereotype(p.getProperty("template.stereotype." + i + ".name",
		 * "" )); } catch (NoSuchElementException nsee) { } if (stm == null) {
		 * String description = p.getProperty("template.stereotype." + i +
		 * ".description", "-").replace("$BR$", "\n"); stm =
		 * this.createStereotype(name, description); if
		 * (Boolean.parseBoolean(p.getProperty("template.stereotype." + i +
		...
		for (int i = 0; i < len; i++) {
			...
			if ((dbt != null) && (name.length() > 0)) {
				...
				if (dom == null) {
					dom = this.getDomain(name, DBType.Convert(dbt), this.getInteger(p, "template.domain." + i + ".vks",
							0), this.getInteger(p, "template.domain." + i + ".nks", 0), d);
		...
		for (int i = 0; i < len; i++) {
			...
			if ((name.length() > 0) && (dom != null)) {
				TabellenspaltenModel tsm = this.createTabellenspalte(name, dom, Boolean.parseBoolean(p
						.getProperty("template.column." + i + ".primarykey")));
				...
				tsm.setReferenceWeight(ReferenceWeight.valueOf2(p.getProperty("template.column." + i
						+ ".referenceweight", "keine")));
				tsm.setToolTipText(p.getProperty("template.column." + i + ".tooltiptext", "-"));
				tsm
						.setWriteablemember(Boolean.parseBoolean(p.getProperty("template.column." + i
								+ ".writeablemember")));
				...
				if (Boolean.parseBoolean(p.getProperty("template.column." + i + ".tostring.member"))) {
					tm.addToStringMember(new ToStringContainer(tsm, tm, p.getProperty(
							"template.column." + i + ".tostring.prefix", "").replace("&nbsp;", " "), p.getProperty(
							"template.column." + i + ".tostring.suffix", "").replace("&nbsp;", " ")));
				}
				if (Boolean.parseBoolean(p.getProperty("template.column." + i + ".tocombostring.member"))) {
					tm.addComboStringMember(new ToStringContainer(tsm, tm, p.getProperty(
							"template.column." + i + ".tocombostring.prefix", "").replace("&nbsp;", " "), p
							.getProperty("template.column." + i + ".tocombostring.suffix", "").replace("&nbsp;", " ")));
				}
				...
				if (p.getProperty("template.column." + i + ".referenceto.table", "").length() > 0) {
					...
					if (tm0 != null) {
						TabellenspaltenModel tsm0 = tm0.getTabellenspalte(p.getProperty("template.column." + i
								+ ".referenceto.column"));
						if (tsm0 != null) {
	...
	public TabellenModel createTabelle(ViewModel view, int x, int y, DiagrammModel d, boolean filled /*
		...
		if (this.templatefilename == null) {
			if (filled) {
				...
				TabellenspaltenModel tsmBezeichnung = this
						.createTabellenspalte("Description", domainBezeichnung, false);
				...
				tsmBezeichnung.setToolTipText(StrUtil.FromHTML("Geben Sie hier eine eindeutige,"
						+ "aussagekr&auml;ftige Bezeichnung ein."));
				...
				tsmKuerzel.setToolTipText(StrUtil.FromHTML("Geben Sie hier ein eindeutiges, "
						+ "aussagekr&auml;ftiges K&uuml;rzel ein."));
				...
				try {
					...
					stm = this.createStereotype("Deactivatable", StrUtil.FromHTML("Diese "
							+ "Stereotype kennzeichnet eine Tabelle, deren Datens&auml;tze "
			...
			for (int i = 0; i < len; i++) {
				...
				if (name.length() > 0) {
					...
					try {
						...
						stm = this.createStereotype(name, System.getProperty("template.stereotype." + i
								+ ".description", "-"));
						d.addStereotype(stm);
					}
					tm.addStereotype(stm);
			...
			for (int i = 0; i < len; i++) {
				...
				if ((dbt != null) && (name.length() > 0)) {
					DomainModel dom = this.getDomain(name, DBType.Convert(dbt), Integer.getInteger("template.domain."
							+ i + ".vks", 0), Integer.getInteger("template.domain." + i + ".nks", 0), d);
			...
			for (int i = 0; i < len; i++) {
				...
				if ((name.length() > 0) && (dom != null)) {
					TabellenspaltenModel tsm = this.createTabellenspalte(name, dom, Boolean
							.getBoolean("template.column." + i + ".primarykey"));
					...
					if (Boolean.getBoolean("template.column." + i + ".tostring.member")) {
						tm.addToStringMember(new ToStringContainer(tsm, tm, System.getProperty(
								"template.column." + i + ".tostring.prefix", "").replace("&nbsp;", " "), System
								.getProperty("template.column." + i + ".tostring.suffix", "").replace("&nbsp;", " ")));
					}
					if (Boolean.getBoolean("template.column." + i + ".tocombostring.member")) {
						tm.addComboStringMember(new ToStringContainer(tsm, tm, System.getProperty(
								"template.column." + i + ".tocombostring.prefix", "").replace("&nbsp;", " "), System
								.getProperty("template.column." + i + ".tocombostring.suffix", "").replace("&nbsp;",
										" ")));
					}
					...
					if (System.getProperty("template.column." + i + ".referenceto.table", "").length() > 0) {
						TabellenModel tm0 = d.getTabelle(System.getProperty("template.column." + i
								+ ".referenceto.table"));
						if (tm0 != null) {
							TabellenspaltenModel tsm0 = tm0.getTabellenspalte(System.getProperty("template.column." + i
									+ ".referenceto.column"));
							if (tsm0 != null) {
	...
	public CodeFactory createCodeFactory(String cls) {
		...
		if ((cls != null) && !cls.isEmpty()) {
			try {
				 ...
				 * + "****************");
				 */
				LOG.error("error while code factory instantiation: " + cls + ", exception: " + e.getMessage());
			}
		} else {
	 ...
	 * @param dt
	 *            Der Types-Datentyp f&uuml;r den Fall, da&szlig; die Domain
	 *            angelegt werden mu&szlig;.
	 * @param len
	 *            Die L&auml;nge des Datenfeldes, falls erforderlich.
	 * @param nks
	 *            Eine Angabe zu den Nachkommastellen der Domain.
	 * @param dm
	 *            Eine Referenz auf das DiagrammModel, in dem Gesucht werden
	 *            soll.
	 * @return Eine bereits vorhandene Domain mit dem angegebenen Namen bzw.
	 *         eine neuerzeugte, falls keine mit dem Namen im DiagrammModel
	 *         gefunden worden ist.
	 */
