...
public class ArtifactGeneratorListConfigurationDialog extends JDialogWithInifile implements ActionListener {
	 ...
	 * @param artifactGenerators A list of artifact generators which are configured by the dialog.
	 ...
	 */
	public ArtifactGeneratorListConfigurationDialog(GUIBundle guiBundle, ArtifactGenerator<?>[] artifactGenerators,
			IndividualPreferences ip, DataModel dataModel, String configFilePath, String codeFactoryVersion) {
		...
		this.setTitle(this.guiBundle.getResourceText("artifact.generators.configuration.title").replace("$VERSION$",
				codeFactoryVersion));
	...
	private JPanel createPathLabelPanel(String basePath) {
		JLabel label = new JLabel(this.guiBundle.getResourceText("artifact.generators.configuration.label.path.label"));
	...
	private JButton createButton(String name) {
		JButton button = new JButton(
				this.guiBundle.getResourceText("artifact.generators." + "configuration.button." + name + ".label"));
	...
	private JPanel createObjectTablePanel() {
		...
		tp.add(this.guiBundle.getResourceText("artifact.objects.tab.tables.label"), createTablesTablePanel());
		tp.add(this.guiBundle.getResourceText("artifact.objects.tab.domains.label"), createDomainsTablePanel());
		p.add(tp);
		return p;
	}
	...
	private JPanel createTablesTablePanel() {
		...
		JTable tableTables = new JTableWithInifile(this.tablesTableModel, this.guiBundle.getInifile(),
	...
	private JPanel createGeneratorsTablePanel() {
		this.artifactGeneratorListTableModel = new ArtifactGeneratorListTableModel(this.artifactGenerators,
				this.guiBundle);
		...
		tableGenerators.setDefaultRenderer(String.class, new TableGeneratorsCellRenderer(this.artifactGenerators));
	...
	private JPanel createDomainsTablePanel() {
		this.domainsTableModel = new DomainsTableModel(this.dataModel.getAllDomains(), this.guiBundle);
		JTable tableDomains = new JTableWithInifile(domainsTableModel, this.guiBundle.getInifile(),
				"DomainsConfigurationTable");
		JPanel panelDomains = new JPanel(new BorderLayout(this.guiBundle.getHGap(), this.guiBundle.getVGap()));
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.buttonCancel) {
		...
		} else if ((e.getSource() == this.buttonSelectDomainsAll) || (e.getSource() == this.buttonSelectDomainsNone)) {
			...
			this.artifactGeneratorListTableModel.setSelectedAll(e.getSource() == this.buttonSelectGeneratorsAll);
			this.repaint();
	...
	/**
	 * Returns a container with all objects which the code is to generate for (e. g. tables, domains and code
	 * generators).
	 *
	 * @return A container with all objects which the code is to generate for (e. g. tables, domains and code
	 *         generators).
	 */
	public ObjectsToGenerateCollection getObjectToGenerateCodeForCollection() {
		...
		for (ArtifactGenerator<?> generator : this.artifactGeneratorListTableModel.getSelectedObjects()) {
			c.add(generator);
		}
		return c;
	}
