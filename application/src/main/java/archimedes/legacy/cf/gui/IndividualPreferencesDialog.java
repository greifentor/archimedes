...
import baccara.gui.generics.EditorPanelCreator;
...
public class IndividualPreferencesDialog extends JDialogWithInifile
	 ...
	 * @changed OLI 10.07.2014 - Added.
	 */
	public IndividualPreferencesDialog(JDialog parent, GUIBundle guiBundle, IndividualPreferences ip,
	...
	private JPanel createEditorPanel() {
		return new EditorPanelCreator(this.guiBundle, this, this).create(
				"code.generators.configuration.individual.preferences", this.componentsByComponent,
				this.componentsByName,
				new ComponentData[] {
						new ComponentData("company.name", baccara.gui.generics.Type.STRING, this.ip.getCompanyName()),
						new ComponentData("user.name", baccara.gui.generics.Type.STRING, this.ip.getUserName()),
						new ComponentData("user.token", baccara.gui.generics.Type.STRING, this.ip.getUserToken()),
	...
	private JPanel createTablePanel() {
		...
		this.tableAlternatePathes = new JTableWithInifile(
				new AlternatePathesTableModel(this.guiBundle, this.alternatePathes), this.guiBundle.getInifile(),
	...
	private JButton createButton(String name) {
		JButton button = new JButton(this.guiBundle.getResourceText(
				"code.generators." + "configuration.individual.preferences.button." + name + ".label"));
	...
	private void transferData() {
		this.ip.setBaseCodePath(((JTextField) this.componentsByName.get("base.code.path")).getText());
		this.ip.setCompanyName(((JTextField) this.componentsByName.get("company.name")).getText());
