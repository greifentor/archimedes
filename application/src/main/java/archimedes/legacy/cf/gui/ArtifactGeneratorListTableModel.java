...
public class ArtifactGeneratorListTableModel extends AbstractSelectionManagerTableModel<ArtifactGenerator<?>> {
	 ...
	 *
	 * @param artifactGenerators The code generator arrays which is represented by the table model.
	 * @param guiBundle          A bundle with GUI information.
	 */
	public ArtifactGeneratorListTableModel(ArtifactGenerator<?>[] artifactGenerators, GUIBundle guiBundle) {
	...
	@Override
	public boolean isObjectSelectedWhileInitialization(ArtifactGenerator<?> object) {
...
class ArtifactGeneratorComparator implements Comparator<SelectionManager<ArtifactGenerator<?>>> {
	...
	@Override
	public int compare(SelectionManager<ArtifactGenerator<?>> ag0, SelectionManager<ArtifactGenerator<?>> ag1) {
		String agn0 = this.guiBundle.getResourceText(
				"artifact.generators.configuration.name." + ag0.getObject().getClass().getSimpleName() + ".title");
		String agn1 = this.guiBundle.getResourceText(
				"artifact.generators.configuration.name." + ag1.getObject().getClass().getSimpleName() + ".title");
