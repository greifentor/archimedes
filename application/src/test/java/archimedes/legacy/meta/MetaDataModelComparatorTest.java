...
public class MetaDataModelComparatorTest {
	...
	public void testTableAdditionDetectedCorrectly() {
		...
		mdt.addUniqueConstraint(new MetaDataUniqueConstraint("NewTable_Name_key", mdt, new MetaDataColumn[] { mdt
				.getColumn("Name") }));
		AbstractChangeOperation[] r = this.unitUnderTest.compare(mdm, mdb);
		assertNotNull(r);
	...
	public void testTypeDecimalPlaceChangeDetectedCorrectly() {
		MetaDataModel mdm = this.createTestModel();
		MetaDataModel mdb = this.createTestModel();
		mdm.getTable("Book").addColumn(
				new MetaDataColumn(mdm.getTable("Book"), "Price", "Currency", DBType.NUMERIC, 10, 3, false, false,
						null, false));
		mdb.getTable("Book").addColumn(
				new MetaDataColumn(mdb.getTable("Book"), "Price", "Currency", DBType.NUMERIC, 10, 2, false, false,
						null, false));
		AbstractChangeOperation[] r = this.unitUnderTest.compare(mdm, mdb);
		assertNotNull(r);
		assertEquals(1, r.length);
	...
	public void testTypeLengthChangeDetectedCorrectly() {
		...
		MetaDataModel mdb = this.createTestModel();
		mdm.getTable("Book").addColumn(
				new MetaDataColumn(mdm.getTable("Book"), "Price", "Currency", DBType.NUMERIC, 12, 2, false, false,
						null, false));
		mdb.getTable("Book").addColumn(
				new MetaDataColumn(mdb.getTable("Book"), "Price", "Currency", DBType.NUMERIC, 10, 2, false, false,
						null, false));
